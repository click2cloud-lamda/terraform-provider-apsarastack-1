...
func init() {
	resource.AddTestSweepers("apsarastack_router_interface", &resource.Sweeper{
		Name: "apsarastack_router_interface",
...
func testAccCheckRouterInterfaceDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "apsarastack_router_interface" {
			continue
		}
...
func TestAccApsaraStackRouterInterfaceBasic(t *testing.T) {
	var v vpc.RouterInterfaceType
	resourceId := "apsarastack_router_interface.default"
...
func TestAccApsaraStackRouterInterfaceMulti(t *testing.T) {
	var v vpc.RouterInterfaceType
	resourceId := "apsarastack_router_interface.default.2"
	ra := resourceAttrInit(resourceId, testAccRouterInterfaceCheckMap)
