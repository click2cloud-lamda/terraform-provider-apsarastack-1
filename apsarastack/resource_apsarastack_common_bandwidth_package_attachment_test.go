...
func init() {
	resource.AddTestSweepers("apsarastack_common_bandwidth_package_attachment", &resource.Sweeper{
		Name: "apsarastack_common_bandwidth_package_attachment",
...
func TestAccApsaraStackCommonBandwidthPackageAttachmentBasic(t *testing.T) {
	var v vpc.CommonBandwidthPackage
	rand := acctest.RandIntRange(1000, 9999)
	resourceId := "apsarastack_common_bandwidth_package_attachment.default"
...
func TestAccApsaraStackCommonBandwidthPackageAttachmentMulti(t *testing.T) {
	var v vpc.CommonBandwidthPackage
	rand := acctest.RandIntRange(1000, 9999)
	resourceId := "apsarastack_common_bandwidth_package_attachment.default.1"
	ra := resourceAttrInit(resourceId, map[string]string{
		"bandwidth_package_id": CHECKSET,
		"instance_id":          CHECKSET,
	})
...
func testAccCheckCommonBandwidthPackageAttachmentDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "apsarastack_common_bandwidth_package_attachment" {
			continue
		}
...
func testAccCommonBandwidthPackageAttachmentConfigBasic(rand int) string {
	...
	resource "apsarastack_common_bandwidth_package_attachment" "default" {
		bandwidth_package_id = "${apsarastack_common_bandwidth_package.default.id}"
		instance_id = "${apsarastack_eip.default.id}"
	}
	`, rand)
}
...
func testAccCommonBandwidthPackageAttachmentConfigMulti(rand int) string {
	...
	resource "apsarastack_common_bandwidth_package_attachment" "default" {
		count = "${var.number}"
		bandwidth_package_id = "${element(apsarastack_common_bandwidth_package.default.*.id,count.index)}"
		instance_id = "${element(apsarastack_eip.default.*.id,count.index)}"
	}
	`, rand)
}
