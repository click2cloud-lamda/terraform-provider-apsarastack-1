...
func init() {
	resource.AddTestSweepers("apsarastack_common_bandwidth_package", &resource.Sweeper{
		Name: "apsarastack_common_bandwidth_package",
		F:    testSweepCommonBandwidthPackage,
		// When implemented, these should be removed firstly
		Dependencies: []string{
			"apsarastack_common_bandwidth_package_attachment",
		},
	})
}
...
func TestAccApsaraStackCommonBandwidthPackage_PayByTraffic(t *testing.T) {

	var v vpc.CommonBandwidthPackage
	rand := acctest.RandIntRange(1000, 9999)
	resourceId := "apsarastack_common_bandwidth_package.default"
...
func TestAccApsaraStackCommonBandwidthPackage_PayByBandwidth(t *testing.T) {
...
func TestAccApsaraStackCommonBandwidthPackage_Multi(t *testing.T) {
	var v vpc.CommonBandwidthPackage
	rand := acctest.RandIntRange(1000, 9999)
	resourceId := "apsarastack_common_bandwidth_package.default.9"
	ra := resourceAttrInit(resourceId, map[string]string{
		"bandwidth":            "10",
		"name":                 fmt.Sprintf("tf-testAccCommonBandwidthPackage%d", rand),
		"description":          "",
...
func testAccCheckCommonBandwidthPackageDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "apsarastack_common_bandwidth_package" {
			continue
		}
