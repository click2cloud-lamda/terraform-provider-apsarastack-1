...
func init() {
	resource.AddTestSweepers("apsarastack_yundun_bastionhost_instance", &resource.Sweeper{
		Name: "apsarastack_yundun_bastionhost_instance",
...
func testSweepBastionhostInstances(region string) error {
	...
	for {
		...
		if err != nil {
			return WrapErrorf(err, DataDefaultErrorMsg, "apsarastack_yundun_bastionhost", request.GetActionName(), ApsaraStackSdkGoERROR)
		}
	...
	for _, v := range instances {
		...
		releaseReq.Headers = map[string]string{"RegionId": client.RegionId}
		releaseReq.QueryParams = map[string]string{"AccessKeySecret": client.SecretKey, "Product": "Yundun-bastionhost", "Department": client.Department, "ResourceGroup": client.ResourceGroup}
...
func TestAccApsaraStackYundunBastionhostInstance_basic(t *testing.T) {
	var v yundun_bastionhost.Instance
	resourceId := "apsarastack_yundun_bastionhost_instance.default"
	ra := resourceAttrInit(resourceId, bastionhostInstanceBasicMap)

	serviceFunc := func() interface{} {
		return &bastionhostService{testAccProvider.Meta().(*connectivity.ApsaraStackClient)}
	}
		...
		Steps: []resource.TestStep{
			{
				Config: testAccConfig(map[string]interface{}{
					...
					"security_group_ids": []string{"${apsarastack_security_group.default.0.id}"},
			...
			{
				Config: testAccConfig(map[string]interface{}{
					"security_group_ids": []string{"${apsarastack_security_group.default.1.id}"},
				}),
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						"security_group_ids.#": "1",
...
func TestAccApsaraStackYundunBastionhostInstance_Multi(t *testing.T) {
	var v yundun_bastionhost.Instance

	resourceId := "apsarastack_yundun_bastionhost_instance.default.1"
	ra := resourceAttrInit(resourceId, bastionhostInstanceBasicMap)
...
func resourceBastionhostInstanceDependence(name string) string {
	return fmt.Sprintf(
		`data "apsarastack_zones" "default" {
				...
				data "apsarastack_resource_manager_resource_groups" "default"{
					status="OK"
				}
