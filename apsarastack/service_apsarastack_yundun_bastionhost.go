...
const (
	...
	BastionhostRoleName               = "ApsaraStackBastionHostDefaultRole"
...
var bastionhostpolicyRequired = []BastionhostPolicyRequired{
	{
		PolicyName: "ApsaraStackBastionHostRolePolicy",
		PolicyType: "System",
	},
}
...
func (s *bastionhostService) DescribeYundunBastionhostInstance(id string) (v yundun_bastionhost.Instance, err error) {
	...
	request.QueryParams = map[string]string{"AccessKeySecret": s.client.SecretKey, "Product": "Yundun-bastionhost", "Department": s.client.Department, "ResourceGroup": s.client.ResourceGroup}
...
func (s *bastionhostService) createRole() error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, BastionhostRoleName, createRoleRequest.GetActionName(), ApsaraStackSdkGoERROR)
	}
...
func (s *bastionhostService) attachPolicy(policyToBeAttached []BastionhostPolicyRequired) error {
	...
	for _, policy := range policyToBeAttached {
		...
		if err != nil {
			return WrapErrorf(err, DefaultErrorMsg, BastionhostRoleName, attachPolicyRequest.GetActionName(), ApsaraStackSdkGoERROR)
		}
...
func (s *bastionhostService) ProcessRolePolicy() error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, BastionhostRoleName, listPolicyForRoleRequest.GetActionName(), ApsaraStackSdkGoERROR)
	}
...
func (s *bastionhostService) UpdateResourceGroup(resourceId, resourceGroupId string) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, resourceId, request.GetActionName(), ApsaraStackSdkGoERROR)
	}
	addDebug(request.GetActionName(), raw, request.RpcRequest, request)
	return nil
}
