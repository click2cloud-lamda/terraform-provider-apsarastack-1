...
func resourceApsaraStackRouterInterfaceConnection() *schema.Resource {
	return &schema.Resource{
		Create: resourceApsaraStackRouterInterfaceConnectionCreate,
		Read:   resourceApsaraStackRouterInterfaceConnectionRead,
		Delete: resourceApsaraStackRouterInterfaceConnectionDelete,
...
func resourceApsaraStackRouterInterfaceConnectionCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if object.OppositeInterfaceId == oppositeId {
		...
		if object.Status == string(Inactive) {
			...
			return resourceApsaraStackRouterInterfaceConnectionRead(d, meta)
		}
	}
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_router_interface_connection", request.GetActionName(), ApsaraStackSdkGoERROR)
	}
	...
	if object.Role == string(InitiatingSide) {
		...
		if err := resource.Retry(2*time.Minute, func() *resource.RetryError {
			...
			return WrapErrorf(err, DefaultErrorMsg, d.Id(), connectRequest.GetActionName(), ApsaraStackSdkGoERROR)
		}
...
func resourceApsaraStackRouterInterfaceConnectionRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	vpcService := VpcService{client}
...
func resourceApsaraStackRouterInterfaceConnectionDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	vpcService := VpcService{client}
	object, err := vpcService.DescribeRouterInterfaceConnection(d.Id(), client.RegionId)
	if err != nil {
