...
func dataSourceApsaraStackBastionhostInstances() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceApsaraStackBastionhostInstancesRead,
...
func dataSourceApsaraStackBastionhostInstancesRead(d *schema.ResourceData, meta interface{}) error {
	...
	for {
		...
		if err != nil {
			return WrapErrorf(err, DataDefaultErrorMsg, "apsarastack_yundun_bastionhost_instances", request.GetActionName(), ApsaraStackSdkGoERROR)
		}
	...
	for _, instanceId := range instanceIds {
...

		{
			...
			if err != nil {
				return WrapErrorf(err, DataDefaultErrorMsg, "apsarastack_yundun_bastionhost_tags ", request.GetActionName(), ApsaraStackSdkGoERROR)
			}
			addDebug(request.GetActionName(), raw, request.RpcRequest, request)
