...
func dataSourceApsaraStackCrEESyncRules() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceApsaraStackCrEESyncRulesRead,
...
func dataSourceApsaraStackCrEESyncRulesRead(d *schema.ResourceData, meta interface{}) error {
	...
	for {
		...
		if err != nil {
			return WrapErrorf(err, DataDefaultErrorMsg, "apsarastack_cr_ee_sync_rules", request.GetActionName(), ApsaraStackSdkGoERROR)
		}
		addDebug(request.GetActionName(), raw, request.RpcRequest, request)

		response, _ = raw.(*cr_ee.ListRepoSyncRuleResponse)
		if !response.ListRepoSyncRuleIsSuccess {
			return crService.wrapCrServiceError("apsarastack_cr_ee_sync_rules", request.GetActionName(), response.Code)
		}
