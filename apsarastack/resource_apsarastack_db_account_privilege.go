...
func resourceApsaraStackDBAccountPrivilege() *schema.Resource {
	return &schema.Resource{
		Create: resourceApsaraStackDBAccountPrivilegeCreate,
		Read:   resourceApsaraStackDBAccountPrivilegeRead,
		Update: resourceApsaraStackDBAccountPrivilegeUpdate,
		Delete: resourceApsaraStackDBAccountPrivilegeDelete,
...
func resourceApsaraStackDBAccountPrivilegeCreate(d *schema.ResourceData, meta interface{}) error {
	...
	return resourceApsaraStackDBAccountPrivilegeRead(d, meta)
}

func resourceApsaraStackDBAccountPrivilegeRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
		...
		err = resource.Retry(5*time.Minute, func() *resource.RetryError {
			...
			if err != nil {
				if IsExpectedErrors(err, []string{"InternalError", "OperationDenied.DBInstanceStatus"}) {
					return resource.RetryableError(WrapErrorf(err, DefaultErrorMsg, object.DBInstanceId, request.GetActionName(), ApsaraStackSdkGoERROR))
				}
				return resource.NonRetryableError(WrapErrorf(err, DefaultErrorMsg, object.DBInstanceId, request.GetActionName(), ApsaraStackSdkGoERROR))
			}
...
func resourceApsaraStackDBAccountPrivilegeUpdate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	rdsService := RdsService{client}
	d.Partial(true)
...
func resourceApsaraStackDBAccountPrivilegeDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	rdsService := RdsService{client}
	parts, err := ParseResourceId(d.Id(), 3)
	if err != nil {
